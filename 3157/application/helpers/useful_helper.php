...
if( !function_exists( 'cms_getNamecategorybyID') ){
	function cms_getNamecategorybyID( $id ){
		$name = 'Chưa có';
		$CI    =& get_instance();
		$category = $CI->db->select( 'prd_cat_name' )->from( 'products_category' )->where( 'ID', $id )->get()->row_array();
		if ( isset( $category ) && count( $category ) ) {
			$name = $category[ 'prd_cat_name' ];
		}
...
if( !function_exists( 'cms_getNamemanufacturebyID') ){
    function cms_getNamemanufacturebyID( $id ){
        $name = 'Không nhập';
        $CI    =& get_instance();
        $customer = $CI->db->select( 'manuf_name' )->from( 'manufactures' )->where( 'ID', $id )->get()->row_array();
        if ( isset( $customer ) && count( $customer ) ) {
            $name = $customer['manuf_name'];
        }
...
if( !function_exists( 'cms_getNamestatusbyID') ){
    function cms_getNamestatusbyID( $id ){
        $name = "";
        switch( $id ){
            case '1': { $name = 'Hoàn thành'; break; }
            case '2': { $name = 'Lưu tạm'; break; }
            case '0': { $name = 'Lưu tạm'; break; }
        }
        return $name;
    }
}
...
if( !function_exists( 'cms_getNamestockbyID') ){
    ...
    function cms_getNamestockbyID( $id ){
        $name = "không xác định";
        $CI    =& get_instance();
        $customer = $CI->db->select( 'stock_name' )->from( 'stocks' )->where( 'ID', $id )->get()->row_array();
        if ( isset( $customer ) && count( $customer ) ) {
...
if( !function_exists( 'cms_currency_format') ){
    function cms_currency_format( $priceFloat ){
